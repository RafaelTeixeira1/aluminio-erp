                        ...
                        <div class="flex items-center space-x-3 md:space-x-6">
                            @php
                                ...
                                if (\Illuminate\Support\Facades\Schema::hasTable('user_notifications')) {
                                    $notificationService = app(\App\Services\OperationalNotificationService::class);
                                    $notificationService->syncForUser(Auth::user());
                                    $pendingNotifications = $notificationService->unreadCount(Auth::user());
                                }
                            @endphp
                            <a href="{{ route('notifications.index') }}" class="relative p-2 text-gray-600 hover:text-gray-900 transition" title="Notificações">
